					...
					if ($this->config->recruits_alert && $this->user->id)
					{
						...
						if ($recipients)
						{
							...
							$reply_id = $this->db	->insert('nf_users_messages_replies', [
														...
														'message'    => '<div class="alert alert-info no-margin"><b>Message automatique.</b><br />Une nouvelle candidature vient d\'être déposée par '.($this->user->id ? $user['username'] : $post['pseudo']).'.<br /><br />Pour la visualiser, <a href="'.url('admin/recruits/candidacy/'.$candidacy_id.'/'.url_title($title)).'">cliquer ici</a>.</div>'
													]);
