			...
			$this->col(
				$this	->panel()
						...
						->body(function($data) use ($image_id, $title, $description, $thumbnail_file_id){
							return '<a class="thumbnail thumbnail-link no-margin" data-toggle="tooltip" title="'.$this->lang('Visualiser').'" data-image-id="'.$image_id.'" data-image-title="'.url_title($title).'" data-image-description="'.$description.'"><img src="'.path($thumbnail_file_id).'" alt="" /></a>';
						})
