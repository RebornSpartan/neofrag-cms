	...
	public function index()
	{
		...
		return [
			$this->row(
				...
				$this->col(
					$this	->panel_box()
							->heading($this->lang('Membre|Membres', $count = $this->db->select('COUNT(*)')->from('nf_users')->where('deleted', FALSE)->row()), 'fa-users', 'admin/user')
							->body($count)
							->color('bg-green')
							->size('col-md-4 col-lg-2')
				...
				$this->col(
					$this	->panel_box()
							->heading($this->lang('Commentaire|Commentaires', $count = $this->db->select('COUNT(*)')->from('nf_comments')->row()), 'fa-comments-o', 'admin/comments')
							->body($count)
							->color('bg-maroon')
							->size('col-md-4 col-lg-2')
