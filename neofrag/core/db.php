...
class Db extends Core
{
	...
	public function __construct($config)
	{
		...
		$this->debug->bar('database', function(&$label){
			...
			{
				$result .= '	<tr>
									<td class="col-1"><b>'.($i + 1).'</b><div class="pull-right"><span class="label label-'.(!empty($request->error) ? 'danger' : 'success').'">'.round($request->time * 1000, 3).' ms</span></div></td>
			...
			$label = '<span class="label label-'.($total_errors > 0 ? 'danger' : 'success').'">'.($total_errors ?: $i + 1).'</span>';
