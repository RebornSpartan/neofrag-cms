...
abstract class Labelable extends Library
{
	...
	public function __toString()
	{
		...
		foreach ($this->_template as $i => $template)
		{
		...
		if (!($display & \NF\NeoFrag\Libraries\Form2::FORM_COMPACT))
		{
			...
			if ($display & \NF\NeoFrag\Libraries\Form2::FORM_INLINE)
			{
				$label .= '&nbsp;';
			}
		}
		...
		if (!($display & \NF\NeoFrag\Libraries\Form2::FORM_INLINE))
		{
			...
			{
				$input = parent	::html()
								->attr_if($this->_size, 'class', $this->_size)
								->content($input);
			}
		}
